    ...
    <el-form label-width="100px">
      ...
      <el-form-item>
        <el-button
          ...
          "
        >
          <el-icon class="addr-icon"><plus /></el-icon>添加类型
        </el-button>
      </el-form-item>
      <el-row type="flex" style="flex-wrap: wrap">
        <el-col :md="12" v-for="(item, i) in data.model.components" :key="i">
          ...
          <el-form-item label="脂肪">
            <el-input  v-model="item.fat"></el-input>
            </el-form-item>
          <el-form-item label="碳水化合物">
            <el-input  v-model="item.carbohydrate"></el-input>
            </el-form-item>
...
async function save(): Promise<void> {
  if (props.id) {
    await http.put(`/currency/foods/${props.id}`, data.model);
  } else {
    await http.post("/currency/foods", data.model);
  }
...
async function getEditName(): Promise<void> {
  const res = await http.get(`/currency/foods/${props.id}`);
  data.model = res.data;
}
